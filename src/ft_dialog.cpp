...
namespace ft
{
    Decision::Decision(bn::vector<bn::string_view, 4> options)
     : _options(options){}
            
    DialogLine::DialogLine(Speaker speaker, Emotion emotion, bn::string_view text)
     : _speaker(speaker), _emotion(emotion), _text(text) {}

    DialogLine::DialogLine(Speaker speaker, Emotion emotion, bn::string_view text, Decision decision)
     : _speaker(speaker), _emotion(emotion), _text(text), _decision(decision) {}
    ...
    :  _dialog_lines(&dialog_lines), _text_generator(&text_generator), _dialog_bgs({
            bn::sprite_items::dialog_bg.create_sprite(-80, -44, 0),
            bn::sprite_items::dialog_bg.create_sprite(-32, -44, 1),
            bn::sprite_items::dialog_bg.create_sprite(32, -44, 1),
            bn::sprite_items::dialog_bg.create_sprite(80, -44, 0)
        ...
        _dialog_bgs[3].set_horizontal_flip(true);
    ...
    void Dialog::update(){
        if(!_done){
            ...
            for(int i = 0; i < lines.size(); ++i){
                if(i == 0){
                    _text_generator->generate(-56, -60, lines.at(0), _top_text_sprites);
                } else if (i == 1){
                    _text_generator->generate(-56, -44, lines.at(1), _middle_text_sprites);
                } else if (i == 2){
                    _text_generator->generate(-56, -28, lines.at(2), _bottom_text_sprites);
                }
            }
        } else {
            _top_text_sprites.clear();
